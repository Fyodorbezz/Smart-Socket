...
#ifdef BUF_INITIALISED
	extern int buff[128][128];
	extern int buff2[128][128];
#else
	#define BUF_INITIALISED
	int buff[128][128];
	int buff2[128][128];
#endif
...
void TFT_ILI9163C::check_buff() {
	for(int i=0; i<128; i++){
		for(int j=0; j<128; j++){
			if(buff[i][j] != buff2[i][j]){
				drawPixel(i, j, buff2[i][j]);
			}
		}
	}
}
